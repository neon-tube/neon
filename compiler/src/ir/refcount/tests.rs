...
fn an_unused_pointer_parameter_is_released() {
    ...
    assert!(
        ir.contains("release %0"),
        "the unused string is released: {ir}"
    );
}
...
fn a_returned_pointer_is_moved_not_released() {
    ...
    assert!(
        !ir.contains("release"),
        "a moved-out value is not released: {ir}"
    );
}
...
fn a_returned_view_retains_itself_and_releases_its_base() {
    ...
    assert!(
        ir.contains("retain %1"),
        "the returned view is retained: {ir}"
    );
    assert!(
        ir.contains("release %0"),
        "the view's base is released before the ret: {ir}"
    );
    let retain = ir.find("retain %1").unwrap();
    let release = ir.find("release %0").unwrap();
    assert!(
        retain < release,
        "retain the view before releasing what it looks into: {ir}"
    );
}
...
fn a_pointer_used_twice_is_retained_once() {
    ...
    assert_eq!(
        ir.matches("retain").count(),
        1,
        "one duplicate consume -> one retain: {ir}"
    );
}
