...
fn a_receiver_with_no_impl_names_itself() {
    ...
    let DispatchError::NoImpl { uncovered, .. } = err else {
        panic!("{err:?}")
    };
...
fn a_partly_covered_receiver_names_only_the_gap() {
    ...
    assert!(
        e.solver.is_equiv(uncovered, square),
        "the gap is exactly Square"
    );
}
...
fn a_spanning_receiver_is_a_switch_not_a_vtable() {
    ...
    let Resolution::Switch(arms) = &s.resolution else {
        panic!("{:?}", s.resolution)
    };
    assert_eq!(
        arms.len(),
        2,
        "the applicable set is known here; no vtable is needed"
    );
}
...
fn the_return_is_the_union_of_the_applicable_returns() {
    ...
    let mut e = env("record Circle { r: i64 }
         record Square { s: i64 }
         type Shape = Circle | Square
    ...
    assert_eq!(
        s.ret, st,
        "both impls return str, so the call is as precise as a direct one"
    );
}
...
fn the_more_specific_impl_wins_a_nested_overlap() {
    ...
    let circle = ty(&mut e, "Circle");
    let s = resolve(&mut e, "area", None, &[circle], None).expect("resolves");
    let Resolution::Direct(id) = s.resolution else {
        panic!("{:?}", s.resolution)
    };

    let target = e.impls()[id.0].target.expect("a type target");
    assert!(
        e.solver.is_equiv(target, circle),
        "Circle's impl, not Shape's"
    );
}
...
fn a_widened_value_still_takes_the_specific_impl() {
    ...
    assert!(
        has_circle_arm,
        "a Circle value must still find Circle's impl: {arms:?}"
    );
}
...
fn two_protocols_declaring_the_same_method_are_ambiguous() {
    ...
    let DispatchError::Ambiguous { protocols, .. } = err else {
        panic!("{err:?}")
    };
    assert_eq!(
        protocols,
...
fn one_protocol_answering_is_not_ambiguous() {
    ...
    assert_eq!(
        s.protocol,
        e.lookup_protocol(&[], &["A".to_string()]).expect("A")
    );
}
...
fn a_constructor_subject_method_infers_generics_from_every_argument() {
    ...
    assert_eq!(
        s.ret, i,
        "fold's return is the accumulator's type, from `init`"
    );
}
