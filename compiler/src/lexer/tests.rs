...
fn integer_bases_and_separators() {
    assert_eq!(
        toks("0xFF 0o17 0b1010 1_000_000"),
        vec![
            Token::Int(255),
            Token::Int(15),
            Token::Int(10),
...
fn floats_versus_dots() {
    ...
    assert_eq!(
        toks("t.0"),
        vec![Token::Ident("t".into()), Token::Dot, Token::Int(0)]
    );
}
...
fn plain_string() {
    assert_eq!(
        toks(r#""hi""#),
        vec![Token::StrStart, Token::StrText("hi".into()), Token::StrEnd,]
    );
...
fn nfc_does_not_disturb_ascii() {
    ...
    assert_eq!(
        toks("hello_world2"),
        vec![Token::Ident("hello_world2".into())]
    );
}
...
fn test_and_bench_are_keywords() {
    assert_eq!(
        toks(r#"test "adds" { assert_eq(1, 1) }"#),
        vec![
            Token::Test,
            Token::StrStart,
...
fn comments_are_retained_as_trivia() {
    ...
    assert!(l
        .tokens
        .iter()
        .all(|t| !matches!(t.token, Token::StrText(_))));
}
