// Model: a probe does not stop at a tombstone.
//
// THE INVARIANT: removing a key must not make a *different* key unreachable.
//
// This is the classic open-addressing bug. `neon_map_remove` marks the slot DEAD rather
// than EMPTY precisely because a later key may have been placed further along the probe
// chain that runs through it; a probe that treats DEAD as the end of the chain reports
// that later key missing, with no crash and no error. `neon_map_slot` gets this right by
// continuing past DEAD and only remembering the first one as a candidate insert point.
//
// The hash is unconstrained, so this is checked for every hash function at once --
// including the one that puts both keys in the same bucket, which is the only case where
// the bug can bite, and the one an ordinary test is least likely to construct.
//
// Verifies `src/map.c` compiled from source; see rule 1.
//
// VALIDATED BY MUTATION (rule 6). Two deliberate breaks in map.c, each confirmed to fail
// this model and then reverted: making `neon_map_slot` return at a DEAD slot instead of
// recording it and continuing, and making `neon_map_remove` mark the slot NEON_MAP_EMPTY
// instead of NEON_MAP_DEAD. Both fail on "a key is still found after a different key
// ahead of it on the probe chain was removed".
//
...
int main(void) {
    ...
    // Key 1 may now sit anywhere: in its own bucket, or -- when the two hashes collide
    // -- one slot further along the chain that starts at key 0's slot.
    box* r0 = box_new(0);
    m = neon_map_remove(m, &r0);
    pin_len(m, 1);

    box* q1 = box_new(1);
    void* slot = neon_map_find(m, &q1); // borrows
    PROVE(slot != NULL,
          "a key is still found after a different key ahead of it on the probe chain "
          "was removed");
    PROVE((*(box**)slot)->v == 11, "and it still carries its own value");
    box_release((void*)&q1);

    box* q0 = box_new(0);
    PROVE(neon_map_find(m, &q0) == NULL, "the removed key is gone");
    box_release((void*)&q0);

    // A new key must be able to take the tombstone's slot rather than skipping it, or
    // the table loses capacity every time something is removed.
    box* k2 = box_new(2);
    box* v2 = box_new(12);
    ...
    box* q2 = box_new(2);
    PROVE(neon_map_find(m, &q2) != NULL, "a key inserted over a tombstone is found");
    box_release((void*)&q2);

    box* q1b = box_new(1);
    PROVE(neon_map_find(m, &q1b) != NULL,
          "and the key past the tombstone is still reachable afterwards");
    box_release((void*)&q1b);
    return 0;
}
