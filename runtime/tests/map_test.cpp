...
namespace {
int64_t inc_fn(neon_header* env, int64_t v) {
    (void)env;
    return v + 1;
}
void inc_updater(neon_closure f, const void* in, void* out) {
...
TEST(set_get_and_overwrite) {
    ...
    TEST_EXPECT(m->len == 1);
    TEST_EXPECT(*(int64_t*)neon_map_at(m, &k) == 100);

    int64_t v2 = 200;
    m = neon_map_set(m, &k, &v2); // same key: overwrite, not grow
    TEST_EXPECT(m->len == 1);
    TEST_EXPECT(*(int64_t*)neon_map_at(m, &k) == 200);
    neon_release((neon_header*)m);
}
...
TEST(find_and_contains) {
    ...
    int64_t absent = 2;
    TEST_EXPECT(neon_map_find(m, &k) != nullptr);
    TEST_EXPECT(neon_map_find(m, &absent) == nullptr);
...
TEST(remove) {
    ...
    TEST_EXPECT(m->len == 1);
    TEST_EXPECT(neon_map_find(m, &k1) == nullptr);
    TEST_EXPECT(neon_map_find(m, &k2) != nullptr);
...
TEST(grows_past_the_initial_capacity) {
    ...
    TEST_EXPECT(m->len == 100);
    for (int64_t i = 0; i < 100; i++) {
        TEST_EXPECT(*(int64_t*)neon_map_at(m, &i) == i); // every entry survived resizing
    }
    neon_release((neon_header*)m);
}
...
TEST(equality_ignores_insertion_order) {
    ...
    TEST_EXPECT(neon_map_eq(a, b));

    int64_t k = 0, differ = 999;
    b = neon_map_set(b, &k, &differ);
    TEST_EXPECT(!neon_map_eq(a, b)); // one differing value breaks it
...
TEST(string_keys_and_values) {
    ...
    TEST_EXPECT(m->len == 20);
    neon_str probe = neon_str_lit("7", 1);
    TEST_EXPECT(*(int64_t*)neon_map_at(m, &probe) == 7);
    neon_release((neon_header*)m);
}
...
TEST(keys_and_values_lists) {
    ...
    TEST_EXPECT(keys->len == 1);
    TEST_EXPECT(vals->len == 1);
    TEST_EXPECT(*(int64_t*)neon_list_at(keys, 0) == 3);
    TEST_EXPECT(*(int64_t*)neon_list_at(vals, 0) == 30);
