...
TEST(distinct_tags_are_preserved) {
    ...
    TEST_EXPECT(neon_box_tag(a) == 1);
    TEST_EXPECT(neon_box_tag(b) == 2); // each box keeps its own tag
...
TEST(boxing_a_string_releases_it_with_the_box) {
    ...
    TEST_EXPECT(nt_str_is(*(neon_str*)neon_box_payload(v), "boxed"));
    neon_release((neon_header*)v);
}
